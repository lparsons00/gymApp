...
class QuickWorkoutExerciseViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
  
  @IBOutlet weak var tableView: UITableView!
  
  var exercises: [NSManagedObject] = []
  
  let cellReuseIdentifier = "quickWorkoutExerciseCell"
  ...
  override func viewDidLoad() {
    ...
    title = " Quick Workout Exercise List"
  ...
  @IBAction func addSelectedExercises(_ sender: Any) {
    ...
    delegate?.userDidEnterInformation(info: "gtfo2")
    delegate?.exercisesAdded(exercisesAdd: SelectedExercises)

    //Todo: Go back to the previous view controller?
  }
  
  // Making this a weak variable, so that it won't create a strong reference cycle
  weak var delegate: QuickWorkoutDataEnteredDelegate? = nil
  
}

// Protocol used for sending data back
protocol QuickWorkoutDataEnteredDelegate: AnyObject {
